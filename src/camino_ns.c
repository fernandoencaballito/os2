    ...
    while ( true )
    {
        ...
        switch ( msg )
        {
            ...
            case LEFTBOUND_BARRIER:
                --leftbound_cars;
                ++in_road;
                if ( passed < ((GlobalVars *)mt_curr_task->data)->pass_max )
                    ++passed;
                break;
            ...
            case LEFTBOUND_BUSY:
                if ( !leftbound_cars && !rightbound_cars && !in_road )
                {
                    ...
                    state = IDLE;
                            ...
                            passed == ((GlobalVars *)mt_curr_task->data)->pass_max) )
                {
                    close_barrier(LEFTBOUND);
                    if ( in_road )
...
car(void *arg){
    ...
    for ( pos = 0 ; pos < ROAD_LEN ; pos++ )
    {
        ...
        if ( pos == SENSOR1_POS )
            Send(((GlobalVars *)mt_curr_task->data)->ctl, &car_in, sizeof car_in);
        else if ( pos == BARRIER_POS )
            Send(((GlobalVars *)mt_curr_task->data)->ctl, &car_barrier, sizeof car_barrier);
        else if ( pos == SENSOR2_POS )
            Send(((GlobalVars *)mt_curr_task->data)->ctl, &car_out, sizeof car_out);
        if ( pos )
...
camino_ns_main(int argc, char **argv){
    ...
    gv->ncars = 0;
    gv->pass_max = argc > 1 ? atoi(argv[1]): PASS_MAX;
    ...
    init_road();
    gv->ctl = CreateTask(control, 0, NULL, "control", DEFAULT_PRIO);
    SetData(gv->ctl, gv);
    Ready(gv->ctl);
