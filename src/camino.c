...
get_road(DIRECTION dir, int pos){
    return ((GlobalVars *)mt_curr_task->data)->road[dir][pos];
}
...
set_road(DIRECTION dir, int pos, int value, unsigned fg, unsigned bg){
    ((GlobalVars *)mt_curr_task->data)->road[dir][pos] = value;
...
static void
put_msg(MESSAGE msg){
    WaitSem(((GlobalVars *)mt_curr_task->data)->mail_empty);
    ((GlobalVars *)mt_curr_task->data)->mail = msg;
    SignalSem(((GlobalVars *)mt_curr_task->data)->mail_full);
}
...
get_msg(void){
    ...
    WaitSem(((GlobalVars *)mt_curr_task->data)->mail_full);
    msg = ((GlobalVars *)mt_curr_task->data)->mail;
    SignalSem(((GlobalVars *)mt_curr_task->data)->mail_empty);
    return msg;
}
...
car(void *arg){
    ...
    ((GlobalVars *)mt_curr_task->data)->ncars++; // suponemos que es atómico
    ...
    ((GlobalVars *)mt_curr_task->data)->ncars--;
}

static void
send_car(DIRECTION dir){
    static DIRECTION left = LEFTBOUND, right = RIGHTBOUND;
    Task_t *t = CreateTask(car, 0, dir == LEFTBOUND ? &left : &right, 
                "car", DEFAULT_PRIO);
    SetData(t, mt_curr_task->data);
    Ready(t);
}
...
camino_main(void){
    ...
    gv->mail_empty = CreateSem("mail empty", 1);
    gv->mail_full = CreateSem("mail full", 0);
    gv->ncars = 0;
    ...
    init_road();
    ctl = CreateTask(control, 0, NULL, "control", DEFAULT_PRIO);
    SetData(ctl, gv);
    Ready(ctl);
    ...
    DeleteTask(ctl);
    DeleteSem(gv->mail_empty);
    DeleteSem(gv->mail_full);
    Free(gv);
