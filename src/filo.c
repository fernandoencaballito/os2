...
philosopher(void *arg)
{
    ...
    while ( true )
    {
        ...
        EnterMonitor(((GlobalVars *)mt_curr_task->data)->mon);
        while ( !forks_avail(i) )
            WaitCondition(((GlobalVars *)mt_curr_task->data)->cond[i]);
        take_forks(i);
        LeaveMonitor(((GlobalVars *)mt_curr_task->data)->mon);
        ...
        leave_forks(i);
        if ( forks_avail(left) )
            SignalCondition(((GlobalVars *)mt_curr_task->data)->cond[left]);
        if ( forks_avail(right) )
            SignalCondition(((GlobalVars *)mt_curr_task->data)->cond[right]);
        LeaveMonitor(((GlobalVars *)mt_curr_task->data)->mon);
    }
}
